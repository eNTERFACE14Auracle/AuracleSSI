...
ssi_char_t *AudioAsio::ssi_log_name = "audioasio_";

AudioAsio::AudioAsio(const ssi_char_t* file) 
	: _file(NULL), 
	_pData(NULL),  
	_offsets (0), 
	_selected (0), 
...
bool AudioAsio::setProvider(const ssi_char_t *name, ssi::IProvider *provider)
{
	SSI_ASSERT(provider);
	...
	if (strcmp (name, SSI_AUDIOASIO_PROVIDER_NAME) != 0) {
...
bool AudioAsio::connect()
{
	...
	status = Pa_OpenStream(&_stream, &params, NULL, _options.sr,
		_options.block, paClipOff, &AudioAsio::PaStreamCallback, _pData);
...
bool AudioAsio::supportsReconnect()
{
	return false;
}

int AudioAsio::VPaStreamCallback(const void *input, void *output, unsigned long frameCount, 
...
int AudioAsio::PaStreamCallback(const void *input, void *output, unsigned long frameCount, 
								const PaStreamCallbackTimeInfo *timeInfo, 
								PaStreamCallbackFlags statusFlags, void *userData)
{
	PAudioAsioData pData = ssi_pcast(AudioAsioData, userData);
	return pData->Instance->VPaStreamCallback(input, output, frameCount, timeInfo, statusFlags, userData);	
}

bool AudioAsio::Parse (const ssi_char_t *indices) {
...
int AudioAsio::getDevice (const ssi_char_t *name) {
...
unsigned int AudioAsio::numDevicesForApi(int api_type)
{
	PaDeviceIndex n_devices = Pa_GetDeviceCount();
	const PaDeviceInfo *device;
...
int AudioAsio::LetUserSelectDevice (StringList &list) {
...
int AudioAsio::LetUserSelectDeviceOnConsole(StringList &list) {

...
#ifndef SSI_AUDIO_AUDIOASIO_H
#define SSI_AUDIO_AUDIOASIO_H
...
#define SSI_AUDIOASIO_PROVIDER_NAME "audio"

namespace ssi
{

class AudioAsio;
typedef struct tagAudioAsioData
{
	AudioAsio*	Instance;
	float*		Data;
} AudioAsioData, *PAudioAsioData;

class StringList;
class IProvider;
class AudioAsio : public ISensor
{
...
public:
	...
	class AudioChannel : public IChannel {
		...
		public:
			...
			const ssi_char_t *getName () { return SSI_AUDIOASIO_PROVIDER_NAME; };
...
public:
	...
	static const ssi_char_t *GetCreateName () { return "ssi_sensor_AudioAsio"; }
	const ssi_char_t *getInfo () { return "Opens multiple audio inputs using ASIO interface for recording and streaming."; }
	static IObject* Create(const ssi_char_t* file) { return new AudioAsio(file); }
...
protected:
	...
	AudioAsio(const ssi_char_t* file);
	~AudioAsio();

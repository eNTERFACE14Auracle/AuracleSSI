...
namespace ssi {
	...
	void MicrosoftKinect::setSkeletonProvider (IProvider *skeleton_provider) {
		...
		if (m_skeleton_provider) {			
			...
			SSI_SKELETON_META m;
			m.type = SSI_SKELETON_TYPE::MICROSOFT_KINECT;
			m.num = m_n_skeletons;
	...
	void MicrosoftKinect::setFacePointProvider (IProvider *face_provider) {
		...
		if (m_face_provider) {
			SSI_FACE_META m;
			m.type = SSI_FACE_TYPE::MICROSOFT_KINECT;
			m.num = m_n_skeletons;
	...
	bool MicrosoftKinect::connect () {
		...
		if (m_au_provider || m_face_provider || m_face3d_provider  || m_head_provider) {
			...
			if (FAILED(hr))
			{
				...
				ssi_wrn ("Could not initialize face tracker (%s). hr=0x%x", path, hr);
				return false;
			}
	...
	void MicrosoftKinect::processSkeleton () {
		...
		if (m_skeleton_provider || m_skeleton_to_screen_provider) 
		{
				...
				for(ssi_size_t j=0; j < m_n_skeletons; j++)
				{
					...
					if (m_skeleton_to_screen_provider) {
						...
						for (ssi_size_t i=0; i < SKELETON_JOINT::NUM; i++)
						{
							...
							x = ssi_cast (float, KinectUtils::SkeletonToScreen(skeletonFrame.SkeletonData[pick].SkeletonPositions[i], _options.screenWidth, _options.screenHeight, 320, 240).x);							    
							m_skeleton_to_screen[j][i][SKELETON_JOINT_VALUE::POS_X] = _options.screenScale ? x / _options.screenWidth : x;
							y = ssi_cast (float, KinectUtils::SkeletonToScreen(skeletonFrame.SkeletonData[pick].SkeletonPositions[i], _options.screenWidth, _options.screenHeight, 320, 240).y);
							m_skeleton_to_screen[j][i][SKELETON_JOINT_VALUE::POS_Y] = _options.screenScale ? y / _options.screenHeight : y;
		...
		if (m_au_provider || m_face_provider || m_face3d_provider  || m_head_provider) {
			...
			if (m_LastFaceTrackSucceeded)
			{
				...
				if(m_face_provider && hasfacemodel)
				{
					...
					{
						m_facepoints[i][FACEPOINT_VALUE::X] = _options.screenScale ? pPts2D[i].x / _options.screenWidth : pPts2D[i].x;
						m_facepoints[i][FACEPOINT_VALUE::Y] = _options.screenScale ? pPts2D[i].y / _options.screenHeight : pPts2D[i].y;
					}
				}
	...
	void MicrosoftKinect::showFace (FLOAT* pSU, IFTModel* ftModel) {
		...
		{
			KinectUtils::paintFacePoints (m_colorImage, m_facepoints, true, 0x00FFFF00);
		}
		else
		{

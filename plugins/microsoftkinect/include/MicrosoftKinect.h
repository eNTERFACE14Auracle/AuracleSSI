...
#define SSI_MICROSOFTKINECT_CHANNEL_NUM 10
...
public:
	...
	public:
		...
		Options ()
			...
			simpleFaceTracking (true), nearTrackingMode (true), screenWidth (640), screenHeight (480), screenDepthMultiplier (2000.0f), screenScale (true), rotQuaternion(false), rgbres(RESOLUTION::RES_640x480), depthres(RESOLUTION::RES_640x480),
			deviceIndex (0) {
			...
			addOption ("screenWidth", &screenWidth, 1, SSI_SIZE, "screen width (if skeleton is projected)");
			addOption ("screenHeight", &screenHeight, 1, SSI_SIZE, "screen height (if skeleton is projected)");
...
public:
	...
	static const ssi_char_t *GetCreateName () { return "ssi_sensor_MicrosoftKinect"; };

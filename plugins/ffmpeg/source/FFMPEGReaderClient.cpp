...
bool FFMPEGReaderClient::open () {
	...
	if (_mode != FFMPEGReader::MODE::AUDIO) {
		...
		_video_frame_rgb = avcodec_alloc_frame();
		avpicture_alloc ((AVPicture *) _video_frame_rgb, PIX_FMT_BGR24, _video_codec_context->width, _video_codec_context->height);
		...
		_video_convert_ctx = sws_getCachedContext (NULL,w,h,_video_codec_context->pix_fmt,w,h,PIX_FMT_BGR24,SWS_BILINEAR,NULL,NULL,NULL);
...
bool FFMPEGReaderClient::close () {	
	...
	if(_fmt_ctx){
		...
		if(_video_frame){
			avcodec_free_frame(&_video_frame);
			_video_frame=0;
		}
		if(_audio_frame){
			avcodec_free_frame(&_audio_frame);
			_audio_frame=0;
		}
		if(_video_frame_rgb){
			avpicture_free ((AVPicture *)_video_frame_rgb);
			avcodec_free_frame (&_video_frame_rgb);
			_video_frame_rgb = 0;
		}
...
int FFMPEGReaderClient::decodePacket () {
	...
	if (_mode != FFMPEGReader::MODE::VIDEO && _packet.stream_index == _audio_stream_idx) {
		avcodec_get_frame_defaults (_audio_frame);

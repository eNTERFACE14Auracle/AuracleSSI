...
public:
	...
	public:
		...
		Options () {
			...
			addOption ("url", url, SSI_MAX_CHAR, SSI_CHAR, "output url");
			addOption ("format", format, SSI_MAX_CHAR, SSI_CHAR, "default output format name (if not determined from url)");
...
public:
	...
	static const ssi_char_t *GetCreateName () { return "ssi_consumer_FFMPEGWriter"; };

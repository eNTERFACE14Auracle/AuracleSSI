...
bool FileStreamIn::open (ssi_stream_t &data,
	...
	_stream.dim = ssi_cast (int, sample_dimension);
	...
	_stream.byte = ssi_cast (int, sample_byte);
	...
	*meta = 0;
	...
	if (element && strcmp (element->Value (), "meta") == 0) {
		...
		switch (_stream.type) {
			...
			default:
				ssi_wrn ("type '%s' doesn't support meta information", SSI_TYPE_NAMES[_stream.type]);
				break;
		}
	}

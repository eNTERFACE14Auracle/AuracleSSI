...
File::VERSION FileSamplesOut::DEFAULT_VERSION = File::V2;
...
bool FileSamplesOut::write (ssi_sample_t &data) {
	...
	if (_console) {
		...
		sample.SetAttribute ("class", data.class_id);
		sample.SetAttribute ("user", data.user_id);		
		sample.SetDoubleAttribute ("from", data.time);
		ssi_time_t sr = data.streams[0]->sr;
		if (sr > 0) {
			sample.SetDoubleAttribute ("to", data.time + data.streams[0]->num / data.streams[0]->sr);
		} else {
			sample.SetDoubleAttribute ("to", data.time);
		}
		sample.SetDoubleAttribute ("prob", data.prob);*/
		...
		for (ssi_size_t i = 0; i < _n_streams; i++) {
			...
			item.SetAttribute ("byte", _file_data->tell ());
			item.SetAttribute ("num", stream.num);
			sample.InsertEndChild (item);*/
		}

		//sample.Print (_file_info->getFile (), 1);

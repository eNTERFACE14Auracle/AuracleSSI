...
Factory::dll_handle_map_t APIGenerator::_dll_handle_map;
Factory::object_create_map_t APIGenerator::_object_create_map;

void APIGenerator::SaveCurrentComponentList () {	 
	_dll_handle_map = Factory::_factory->_dll_handle_map;
	_object_create_map = Factory::_factory->_object_create_map;
}
...
<HTML>\n\
	<HEAD>\n\
		<TITLE>Social Signal Interpretation (SSI) API [v0.9.0]\n\
		</TITLE>\n\
	</HEAD>\n\
		...
		<TABLE WIDTH=\"100%\" HEIGHT=\"100%\">\n\
			<TR>\n\
				<TD ALIGN=\"CENTER\" VALIGN=\"MIDDLE\">\n\
					...
					<P>Built with Social Signal Interpretation (SSI) [v0.9.0]\n\
						<BR />(c) 2007-13 University of Augsburg\n\
						<BR />\n\
						<A HREF=\"http://hcm-lab.de/ssi.html\" TARGET=\"new\">http://hcm-lab.de/ssi.html</A>\n\
					</P>\n\
				</TD>\n\
			</TR>\n\
		</TABLE>\n\
...
bool APIGenerator::CreateAPI (const ssi_char_t *filepath) {
	...
	if (_object_create_map.size () == 0) {
		SaveCurrentComponentList ();
	}

	Factory::dll_handle_map_t::iterator it;
	for (it = _dll_handle_map.begin (); it != _dll_handle_map.end (); it++) {	
		head_title.InsertEndChild (TiXmlText (it->first.str ()));	
		head_title.InsertEndChild (TiXmlText (" "));
	}
	...
	cp_p.InsertEndChild (TiXmlText ("(c) 2007-13 University of Augsburg"));
	cp_p.InsertEndChild (cp_p_br);
	TiXmlElement cp_p_a ("A");
	cp_p_a.SetAttribute ("HREF", "http://hcm-lab.de/ssi.html");
	cp_p_a.SetAttribute ("TARGET", "new");
	cp_p_a.InsertEndChild (TiXmlText ("http://hcm-lab.de/ssi.html"));
...
bool APIGenerator::CreateSource (TiXmlElement &body) {
	...
	for (it = _dll_handle_map.begin (); it != _dll_handle_map.end (); it++) {	
		...
		p_dl_dd_font_code_a.SetAttribute ("NAME", it->first.str ());
		p_dl_dd_font_code_a.InsertEndChild (TiXmlText (it->first.str ()));
...
bool APIGenerator::CreateSummary (TiXmlElement &body) {
	...
	Factory::object_create_map_t::iterator it;
	ssi_size_t i = 0; 
	for (it = _object_create_map.begin (); it != _object_create_map.end (); it++) {	
		...
		{			
			...
			TiXmlElement table_tr_td_code ("CODE");
			table_tr_td_code.InsertEndChild (TiXmlText (it->first.str ()));	
			table_tr_td.InsertEndChild (table_tr_td_code);
			table_tr.InsertEndChild (table_tr_td);
		}

		{
			...
			TiXmlElement table_tr_td_code ("CODE");
			TiXmlElement table_tr_td_code_a ("A");
			ssi_char_t *create = it->first.str ();
...
bool APIGenerator::CreateDetails (TiXmlElement &body) {
	...
	for (it = _object_create_map.begin (); it != _object_create_map.end (); it++) {	
		...
		TiXmlElement table_tr_td_code_a ("A");		
		table_tr_td_code_a.SetAttribute ("NAME", it->first.str ());
		table_tr_td_code_a.InsertEndChild (TiXmlText (it->first.str ()));
		table_tr_td_code.InsertEndChild (table_tr_td_code_a);
		table_tr_td.InsertEndChild (table_tr_td_code);
		table_tr.InsertEndChild (table_tr_td);
		TiXmlElement table_tr_td2 ("TD");
		CreateClass (table_tr_td2, it->first.str ());

...
XMLPipeline::VERSION DEFAULT_VERSION = XMLPipeline::V1;
...
bool XMLPipeline::parseTransformer (TiXmlElement *element) {
	...
	if (xinput) {
		...
		int input_size = 0;
		xinput->Attribute ("size", &input_size);
		xinput_size = ssi_cast (ssi_size_t, input_size);

		SSI_DBG (SSI_LOG_LEVEL_DEBUG, "transformer->xinput: <size='%d'>", xinput_size);	
...
bool XMLPipeline::parseConsumer (TiXmlElement *element) {
	...
	if (xinput) {
		...
		int xinput_size;
		xinput->Attribute ("size", &xinput_size);

		SSI_DBG (SSI_LOG_LEVEL_DEBUG, "consumer->xinput: <size='%d'>", xinput_size);	

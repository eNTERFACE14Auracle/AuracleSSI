...
void SocketSpy::enter () {
	...
	if (_opts.host && _opts.host[0] != '\0') {
		_socket = Socket::CreateAndConnect (_opts.protocol, Socket::RECEIVE, _opts.port, _opts.host);
	} else {
		_socket = Socket::CreateAndConnect (_opts.protocol, Socket::RECEIVE, _opts.port);
	}
			...
			if (_n_fork > 0) {
				...
				for (ssi_size_t i = 0; i < _n_fork; i++) {
					...
					if (ssi_split_string_count (url, ':') == 2) {
						...
						_fork[i] = Socket::CreateAndConnect (_opts.protocol, Socket::SEND, port, host);
					} else {
...
void SocketSpy::run () {
	...
	switch (_opts.format) {
		...
		case FORMAT::IMAGE:
			...
			if (result > 0) {
				_plot->setData (_buffer, 1);
				ssi_pcast (ThePainter, _painter)->Paint (_plot_id);
			}
...
void SocketSpy::stream (const ssi_char_t *from,
	...
	if (_opts.show_plot) {
		if (sr != _sample_rate || dim != _sample_dimension || bytes != _sample_bytes || type != _sample_type) {
			ssi_err ("when plotting a stream properties must not change");
			return;
		}
		_plot->setData (data, num);
		ssi_pcast (ThePainter, _painter)->Paint (_plot_id);
	}
};
...
void SocketSpy::init_plot () {

	Factory::RegisterDLL ("ssigraphic.dll");	

	_painter = Factory::Create (ThePainter::GetCreateName(), 0, false);	

	if (_opts.format == FORMAT::IMAGE) {

		ssi_char_t string[SSI_MAX_CHAR];
		ssi_sprint (string, "video@%d", _opts.port);
		_plot_id = ssi_pcast (ThePainter, _painter)->AddCanvas (string);
		_plot = new PaintVideo (_opts.vparams, true, true);
		ssi_pcast (ThePainter, _painter)->AddObject (_plot_id, _plot); 

	} else {

		if (_opts.show_plot) {
			ssi_char_t string[SSI_MAX_CHAR];
			ssi_sprint (string, "plot@%d", _opts.port);
			_plot_id = ssi_pcast (ThePainter, _painter)->AddCanvas (string);
			switch (_sample_type) {
				case SSI_CHAR:
					_plot = new PaintData<char> (_sample_dimension, _opts.plot_type);
					break;
				case SSI_UCHAR:
					_plot = new PaintData<unsigned char> (_sample_dimension, _opts.plot_type);
					break;	
				case SSI_USHORT:
					_plot = new PaintData<unsigned short> (_sample_dimension, _opts.plot_type);
					break;
				case SSI_SHORT:
					_plot = new PaintData<short> (_sample_dimension, _opts.plot_type);
					break;
				case SSI_UINT:
					_plot = new PaintData<unsigned int> (_sample_dimension, _opts.plot_type);
					break;	
				case SSI_INT:
					_plot = new PaintData<int> (_sample_dimension, _opts.plot_type);
					break;	
				case SSI_FLOAT:
					_plot = new PaintData<float> (_sample_dimension, _opts.plot_type);
					break;	
				case SSI_DOUBLE:
					_plot = new PaintData<double> (_sample_dimension, _opts.plot_type);
					break;	
			}
		}

...
int main (int argc, char **argv) {
	...
	cmd.addICmdOption ("-buffer", &buffer_size, Socket::DEFAULT_MTU_SIZE, "max buffer size in bytes\t[MTU size]");
	...
	bool show_plot, plot_as_image, log_binary, log_ascii, log_header, log_console;
	...
	cmd.addBCmdOption ("-plot", &show_plot, false, "plot stream \t\t\t[false]");
	cmd.addBCmdOption ("-image", &plot_as_image, false, "plot as image \t\t\t[false]");
	...
	if (cmd.read (argc, argv)) {
		...
		switch (cmd.master_switch) {
			...
			case 3: {
				...
				opts.plot_update = plot_update;
				opts.plot_type = plot_as_image ? PAINT_TYPE_IMAGE : PAINT_TYPE_SIGNAL;				

...
namespace SSIXmlEditor.InputHandler
{
    class XmlEditorLogic
    {
        ...
        {
            m_Model = model;
            m_KeyWordDetector = new SSIXmlEditor.InputHandler.KeyWordDetector(new Action<string>(KeyWordFilterAction));
			m_ModelRepository = modelRepository;
			m_Model.ModelChanged += new EventHandler<SSIXmlEditor.View.SSIModelChangedEventArgs>(m_Model_ModelChanged);

            m_Timer = new Timer(new TimerCallback(Update), null, 1000, 200);
        }

        void m_Model_ModelChanged(object sender, SSIXmlEditor.View.SSIModelChangedEventArgs e)
        {

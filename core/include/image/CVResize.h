...
public:
	...
	public:
		...
		Options () : width (0), height (0), method (LINEAR) {

			addOption ("width", &width, 1, SSI_SIZE, "new width in pixels");		
			addOption ("height", &height, 1, SSI_SIZE, "new height in pixels");		
			addOption ("method", &method, 1, SSI_INT, "interpolation method (0=NN,1=LINEAR,2=CUBIC,3=AREA)");
		};


		void setResize (ssi_size_t width, ssi_size_t height, METHOD method = LINEAR) {
			this->width = width;
			this->height = height;
			this->method = method;
		}
...
public:
	...
	static const ssi_char_t *GetCreateName () { return "ssi_filter_CVResize"; };
